...
class MSample_supply_details extends CI_Model {
  ...
  function get_by_id($id) {
    $data = array();
    $this->db->where('id', $id);
    $q = $this->db->get('sample_supply_details');
    if ($q->num_rows() > 0) {
  ...
  function create() {
    ...
    $this->db->insert('sample_supply_details', $data);
  }

  function delete_by_supply_id($supply_id) {
    $this->db->where('supply_id', $supply_id);
    $this->db->delete('sample_supply_details');
  }
